...
## アルゴリズム解説とスライド (Documentation & Slides)

本プロジェクトの各種アルゴリズムや動作フローの詳細、および解説スライドについては、以下の GitHub Pages ポータルサイトを参照してください。
* 🌐 **[PFS Netflow 観測準備パイプライン ポータル](https://yasudank.github.io/NetflowByUser/)**

このポータルでは以下が公開されています：
* 🇯🇵🇺🇸 **ポインティング・ガイド星最適化アルゴリズム解説** (日本語/英語)
* 🇯🇵🇺🇸 **統合パイプライン動作フロー・ダミーターゲット回避解説** (日本語/英語)
* 🇯🇵🇺🇸 **Marp プレゼンテーション用スライド** (日本語/英語)

---

## クイックスタート (Quick Start)

以下の一連のコマンドを順に実行することで、サンプルデータに対するファイバー割り当てから `pfsDesign` の生成までの一連の処理をすぐにテストすることができます。
...
**【補足】**
* `run_netflow` を引数なしで実行すると、デフォルトで `netflow_pipeline_config.yaml` の設定が読み込まれ、自動で処理が進行します。
* 注意: 実際の最適化計算を行うためには [Gurobi Optimizer のライセンス](#01-gurobi-optimizer-のライセンス準備) が事前に完了している必要があります。
...
* **未アサインかつ明るい星近傍のファイバーに対するダミー天体アサイン（第2段階）**:
  アサイン計算の終了後、未アサインとなった正常なファイバー（Cobra）のうち、周囲 1.5分角以内に Gaia G等級が 12.0 以下の明るい星があるものを特定します。これらのファイバーについて、隣接する他のファイバー（アサイン済みの天体や、他の未アサインファイバーのホームポジションなど）とコリジョンを起こさない範囲で、元の明るい星から最も遠い位置にダミー天体を自動設定（Cobraのパトロール領域内 `r_min + 0.05` 〜 `r_max - 0.05` mm の範囲で極座標グリッド探索）し、そのファイバーをそのダミー天体にアサインした行を `targets/science/{ppc_code}.ecsv` に追記します。
  * 元のファイルは差分確認用に `.orig` という拡張子（例: `{ppc_code}.ecsv.orig`）でバックアップ保存されます。
  * 各ダミー天体の元の明るい星の位置・明るさ（等級）および距離の変化（初期値 vs. 配置後）を記録した詳細なレポートが `targets/science/dummy_target_improvements.txt` に出力されます。
