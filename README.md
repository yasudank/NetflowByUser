...
### 0.2. Calibration catalog (sky, fluxstd) と Guide Start catalog (gaia) の準備
...
# 引数に自身のGitHubユーザー名、リポジトリ名、リリースタグを指定できます
bash download_data.sh <your-github-username> <your-repository-name> v1.0.0-data
```

---

### 0.3. データベース接続設定 (`db_config.toml`)
...
準備したターゲットリストから、指定した priority (default 2) までの天体を、指定した視野数で最大数カバーできるように、greedy のアルゴリズムを使って視野を最適化する。PFSの視野は六角形を仮定し、中心位置と回転角 (PA) を決める。結果は optimized_pointings.ecsv に保存される。
...
* `optimize_hex_fov.py` 内のアルゴリズム（Greedy Maximum Coverage）を使用し、ロードされた `science_targets` のうち `priority <= max_priority` を満たすターゲット天体を最も効率よくカバーするように視野の中心座標と回転角 (PA) を探索・最適化します。
...
* 各天体の露出時間を、設定ファイルに定義された1回あたりの観測露出時間 `t_obs` (デフォルト: `3600.0`s) に一時的に上書きします。これにより、カタログ上の設定と切り離した露出条件での最適配置が計算されます。
