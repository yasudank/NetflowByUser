...
`run_netflow.py` is an end-to-end pipeline script that automatically executes fiber assignment optimization calculations (netflow) for PFS (Prime Focus Spectrograph), followed by the generation and validation of `pfsDesign` FITS files and OPE observation control files. It was developed by referencing programs developed by the Subaru Telescope observatory. Almost all the code was generated by Google Gemini.
